...
When the database becomes inaccessible, it is not worth checking every 5 seconds to see if it has improved, as this will only put an unnecessary load on the system. Instead, we start a timer at the time of the outage and do not re-check the database before a set time has elapsed. The class TimeoutLatch implements the hysteresis.
